...
def _traffic_at(minute: int, scenario: str) -> float:
    ...
    if scenario == "stable":
        ...
        # Gentle wave: light → moderate → light
        base = 1.3
        wave = 0.3 * math.sin(math.pi * minute / 20)
        return round(base + wave, 2)

    elif scenario == "peak_hour":
        # Spike between minutes 10–20 (lunch rush), calmer outside
        if minute < 8:
            return 1.1
        elif minute < 12:
            return 1.1 + (minute - 8) * 0.25     # ramp up
        elif minute < 20:
            return 2.1 + 0.1 * math.sin(minute)  # peak ~2.0–2.2
        else:
            return 2.1 - (minute - 20) * 0.08    # ramp down
    return 1.0
...
# ─────────────────────────────────────────────
ORDER_TEMPLATES = {
    # id, item_name, prep_time_min, initial_driver_dist_km
...
def _make_order(order_id: str, template_key: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "driver_eta_min":        None,    # estimated minutes remaining until arrival
...
def _score_order(order: Dict[str, Any]) -> Tuple[float, str]:
    ...
    # Normalize: best possible = +10, worst practical = -35
    # Range span = 45
...
class KitchenflowEnvironment(Environment):
    ...
    def step(self, action: KitchenAction) -> KitchenObservation:
        ...
        speed   = BASE_SPEED_KM_MIN / traffic     # km per minute
        ...
        for o in self._orders:
            ...
            # 4. Cool food if it's ready and waiting
            if o["food_ready"] and not o["delivered"]:
                if not o["driver_arrived"]:
                    # Food sitting under heat lamp, slowly cooling
                    o["food_temp_c"] = max(
                        40.0, o["food_temp_c"] - COOLING_RATE
                    )
                    o["minutes_food_waited"] += 1
                else:
                    # Both ready → deliver
                    # Cool slightly during final hand-off
                    o["food_temp_c"] = max(40.0, o["food_temp_c"] - 0.5)

            # 5. Driver idle at hub waiting for food
            if o["driver_arrived"] and not o["food_ready"]:
                o["minutes_driver_waited"] += 1

            # 6. Deliver if both ready
    ...
    def _shaped_reward(self) -> float:
        """
        Per-step shaped reward to guide the agent before episode end.
        Rewards progress: food staying warm, efficient driver dispatch.
        """
        reward = 0.0
        for o in self._orders:
            if o["delivered"]:
            ...
            # Small bonus while food is staying warm (not yet ready or just ready)
            if o["food_ready"] and o["food_temp_c"] >= PERFECT_TEMP:
                reward += 0.05

            # Penalty each minute food sits waiting without a driver
            if o["food_ready"] and not o["driver_summoned"]:
                reward -= 0.10

            # Penalty each minute driver sits idle at hub
            if o["driver_arrived"] and not o["food_ready"]:
                reward -= 0.15
